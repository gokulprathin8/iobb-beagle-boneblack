 ...
 *
 * spi_transact provide Tx/Rx data from SPI module .
 *
 ...
 */
int spi_transact(unsigned int SPI_ID, unsigned int chn, unsigned int Tx_data, unsigned int *Rx_data)
{
...
#ifdef BBBIO_LIB_DBG
		printf("spi_transact: McSPI %d CM_PER Clock Gating!\n",SPI_ID);
#endif
		return 0 ;
	}
	return 1 ;
 ...
 * 
 * spi_ctrl provide channel configuration .
 ...
 *	@return : 0 for error , 1 for success .
*/
int spi_ctrl(unsigned int SPI_ID ,		/* SPI module ID , am335x have SPI1 and SPI0*/
				...
				unsigned int WL)		/* WL 			, word length */
{
	...
	if(!McSPI_Module[SPI_ID].CM_PER_enable) {
		printf("spi_ctrl: McSPI %d CM_PER Clock Gated!\n",SPI_ID);
		param_error =1;
	}
	else if(MS == BBBIO_McSPI_Slave) {
		printf("spi_ctrl: Sorry, iobb only for master mode.\n");
		param_error =1;
	}
	else if(WL <4 || WL >32)
		param_error =1;
